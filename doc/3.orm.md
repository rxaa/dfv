...
其属性对应类型与缺省值等等等生成规则，都可以在**config/template/mysqlModel.ts**里修改。
...
```typescript
    //生成了这样的sql语句:select id,name from TestTable where name='名字'
...
[http://mongodb.github.io/node-mongodb-native/2.1/api/Collection.html](http://mongodb.github.io/node-mongodb-native/2.1/api/Collection.html)。
